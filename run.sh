...
start () {
    ...
    docker run -dit -p "$HOST_SAVIME_PORT":65000 -v "$HOST_SAVIME_DIR":/dev/shm/savime -v /tmp:/tmp -u $(id -u ${USER}):$(id -g ${USER}) --name $CONTAINER_NAME -e MAX_THREADS=$MAX_THREADS -d savime > /dev/null 2>&1 
    if [ $? -eq  0 ]; then
